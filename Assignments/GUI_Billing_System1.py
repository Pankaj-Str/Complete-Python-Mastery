...
class BillingSystem:
    def __init__(self, root):
        ...
        # Menu Card
        self.menu = {
            "Item 1": {"name": "Codes With Pankaj 1", "price": 10.00},
            "Item 2": {"name": "Codes With Pankaj 2", "price": 15.00},
            "Item 3": {"name": "Codes With Pankaj 3", "price": 8.50},
    ...
    def create_menu_card(self):
        ...
        for i, (item, details) in enumerate(self.menu.items(), start=1):
            ttk.Label(frame_menu, text=f"{details['name']}: ${details['price']:.2f}").grid(row=i, column=0, sticky="w")

            quantity_var = tk.IntVar()
            ttk.Spinbox(frame_menu, from_=0, to=10, textvariable=quantity_var).grid(row=i, column=1, padx=5)

            add_button = ttk.Button(frame_menu, text="Add", command=lambda item=item, details=details, quantity=quantity_var: self.add_to_cart(item, details, quantity))
    ...
    def add_to_cart(self, item, details, quantity_var):
        quantity = quantity_var.get()
        if quantity > 0:
            if item in self.selected_items:
                self.selected_items[item]["quantity"] += quantity
            else:
                self.selected_items[item] = {"name": details["name"], "price": details["price"], "quantity": quantity}
    ...
    def update_selected_items_label(self):
        ...
        for item, details in self.selected_items.items():
            items_text += f"{details['name']} x{details['quantity']} (${details['price']:.2f})\n"
    ...
    def generate_bill(self):
        ...
        for item, details in self.selected_items.items():
            print(f"{details['name']} x{details['quantity']} (${details['price']:.2f})")
